...
class TestHelper():
    ...
    def _checkEnvVars():
        if not TestHelper.DEVICE_NAME:
            TestHelper.DEVICE_NAME = 'macbook' #raise RuntimeError("Please set DEVICE_NAME env var (no spaces allowed yet)")
        
        if not TestHelper.CHROMIUM_SRC:
            raise RuntimeError("Please set CHROMIUM_SRC env var.")
        
        if not TestHelper.USE_CEF:
            TestHelper.USE_CEF = False #raise RuntimeError("Please set USE_CEF env var.")
